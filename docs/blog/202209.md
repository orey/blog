...
Notamment, si je suis un guerrier non magicien et que `COMP=10`, alors j'ai `10` points à répartir dans ma compétence. Comme `12` est toujours un échec, je peux décider de n'investir que `1` en `Combat à l'épée`, à moins que je ne décide d'investir `4` en prévision de potentiels malus. Mais si le MJ est comme moi et qu'il limite les malus/bonus au strict minimum, au final, le jeu est prévisible, pour les joueurs.
